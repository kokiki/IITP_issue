...
`서류샘플/`에는 실제 검토용 샘플 이미지가 포함되어 있고, `test_integrated.py`는 샘플 전체를 한 번에 업로드해 진행률·판별 결과·HWPX 요약보고서 다운로드를 검증합니다. XLSX exporter는 보조 데이터용으로 남아 있지만 기본 다운로드 결과는 HWPX입니다.
